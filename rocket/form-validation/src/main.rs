...
impl<'v> FromFormValue<'v> for AdultAge {
    ...
    fn from_form_value(v: &'v RawStr) -> Result<Self, Self::Error> {
        ...
        match age > 20 {
            ...
            false => Err("must be at least 21 to enter"),
        }
    }
}
...
fn login(user: Form<UserLogin>) -> Result<Redirect, String> {
    ...
    if user.username == "cyobero" {
        if let Ok(StrongPassword("password")) = user.password {
            Ok(Redirect::to("/user/cyobero"))
        } else {
...
fn rocket() -> rocket::Rocket {
    rocket::ignite().mount("/", routes![files::index, files::files, user_page, login])
}
