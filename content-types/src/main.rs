...
use rocket::{data::Data, response::content, Request};
...
// in a `GET` request and all other non-payload supporting request types, the preferred media type
// in the Accept header is matched against the `format` in the route attribute. Note: if this was a
// real application, we would use `rocket_contrib`'s built-in JSON support and return a
// `JsonValue` instead.
#[get("/<name>/<age>", format = "json")]
fn get_hello(name: String, age: u8) -> content::Json<String> {
    // In a rel application we would use JSON contrib type.
    let person = Person {
        name: name,
        age: age,
    };
...
fn not_found(request: &Request) -> content::Html<String> {
    let html = match request.format() {
        ...
        _ => format!(
            "<p>Sorry, '{} is an invalid path! Try /hello/&lt;name&gt;age&gt; instead. </p>",
            request.uri()
        ),
    };
